...
def create_session(conf: dict) -> tuple:
    ...
    session.sql(f"USE DATABASE {sf['database']}").collect()
    session.sql(f"USE SCHEMA {sf['schema']}").collect()
    session.sql(f"USE ROLE {role}").collect()
    session.sql(f"USE WAREHOUSE {sf['warehouse']}").collect()
    session.sql(f"ALTER WAREHOUSE {sf['warehouse']} SET WAREHOUSE_SIZE = {sf['warehouse_size']}").collect()
